...
Scheduled weekly refresh. Scraped t3-sponsors source (sponsors.ts); found 6 entries not yet in the catalog: Devin, CopilotKit, Render, General Translation, Mercury, and Boot.dev. Scope pre-filter results: Devin, CopilotKit, and Render passed (developer tools you integrate into a project or deploy workflow); General Translation passed the scope filter but has no clean taxonomy match (i18n/localization library for React — closest area is dev-workflow but that area's definition targets code-review flow and pairing, not project-level SDK integration), so no file was created and this is logged for a human to map or to extend taxonomy.md with a new area; Mercury was rejected (business banking product, not a developer tool); Boot.dev was rejected (coding education platform). No queue items to drain. No stale entries: all existing entries carry last-researched dates of 2026-05-21 to 2026-06-01, all within the 90-day re-research window. Added 3 tool entries: Devin (assess, ai-coding-agents) — autonomous AI software engineer for async task delegation, entry at $20/month Core plan; CopilotKit (assess, ai-agent-infra) — MIT-licensed open-source React framework for embedding AI copilots and agent UIs, free self-hosted and a free cloud developer tier; Render (assess, hosting-deploy) — Heroku-style managed PaaS with a real free tier (750 compute hours/month), zero major platform outages in 12 months, bandwidth policy change to note in April 2026. INDEX.md regenerated; catalog now holds 82 tool entries across 22 problem areas: 3 adopt, 9 hold, 70 assess.
