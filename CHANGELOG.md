...
Scheduled weekly refresh. Scraped t3-sponsors source (sponsors.ts); found 16 new candidates not yet in the catalog. Rejected 12 as out of scope: Frontend Masters, Code Crafters, Epic Web, Boot.dev (education/courses); Eight Sleep (lifestyle product); Superhuman, Notion (general productivity apps); Infinite Red, Dockyard (dev shops/consultancies); G2i (hiring marketplace); Mercury (business banking); Fondo (startup accounting service). Note: ten of the twelve (Frontend Masters, Eight Sleep, Superhuman, Code Crafters, Infinite Red, Dockyard, G2i, Notion, Fondo, Epic Web) were already rejected on the 2026-05-22 run and appear again in the source; Mercury and Boot.dev are new rejections. Added 4 tool entries: Devin (assess, ai-coding-agents) — autonomous AI software engineer by Cognition AI with a free tier and $20/month entry plan; CopilotKit (assess, ai-agent-infra) — open-source React/Angular SDK for embedding AI copilots in apps, 28k+ GitHub stars, $27M Series A; Render (assess, hosting-deploy) — Heroku-successor cloud platform with a free tier, preview environments, and SOC 2 compliance; General Translation (assess, dev-workflow) — open-source i18n SDK for React replacing JSON key extraction with a `<T>` wrapper plus an AI translation service. Taxonomy note: no i18n area exists; `dev-workflow` is assigned as the closest fit — a human should evaluate whether an `i18n` or `localization` area should be added to taxonomy.md. No queue items to drain (queue empty). No stale entries requiring re-research (oldest last-researched is 2026-05-21, 39 days ago, well within the 90-day window). INDEX.md regenerated; catalog now holds 83 tool entries across 22 problem areas: 3 adopt, 9 hold, 71 assess.
